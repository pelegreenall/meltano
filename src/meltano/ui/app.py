...
def create_app(ctx: AppContext) -> FastAPI:
    ...
    def index(request: Request) -> Response:
        ...
        token = request.query_params.get(security.TOKEN_QUERY_PARAM)
        if token is not None and security.constant_time_eq(token, ctx.settings.token):
            security.set_token_cookie(response, token)
        return response
...
__all__ = ["DOCS_URL", "OPENAPI_URL", "STATIC_DIR", "create_app"]
